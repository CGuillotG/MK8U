...
let url = process.env.REACT_APP_ENDPOINT+'/edit'
...
export default class ProfileEdit extends React.Component {
    ...
    submit = () => {
        ...
        if (user.username !== updateUser.username || user.email !== updateUser.email) {
            axios
                .post(url, { ...updateUser }, { withCredentials: true })
    ...
    render() {
        ...
        return (
            <div>
                ...
                <h3>
                    Username :{' '}
                    <input name='username' type='text' placeholder={user.username} onChange={this.handleChange} />
                </h3>
                <p>
                    Email: <input name='email' type='text' placeholder={user.email} onChange={this.handleChange} />
                </p>
                <Link to='/profile'>
